...
public class MovieFragment extends Fragment {
  ...
  public View onCreateView(
    ...
    view = inflater.inflate(R.layout.tfe_re_fragment_selection_list, container, false);

    // Set the adapter
    if (view instanceof RecyclerView) {
      Context context = view.getContext();
      recyclerView = (ListView) view.findViewById(R.id.movie_list);

      recyclerView.setAdapter(new CustomMovieListAdapter(getActivity(),items, listener));
    }
    return view;
  }
  ...
  public void setMovies(List<MovieItem> movies) {
    recyclerView = (ListView) view.findViewById(R.id.movie_list);
    this.items = movies;
    recyclerView.setAdapter(new CustomMovieListAdapter(getActivity(),this.items, listener));
  }
  ...
  public void setMovies(List<MovieItem> favoriteMovies, List<RecommendationClient.Result> recommendations) {
    recyclerView = (ListView) view.findViewById(R.id.movie_list);
    this.items = favoriteMovies;
    recyclerView.setAdapter(new CustomMovieListAdapter(getActivity(),this.items, listener,recommendations));
  }
