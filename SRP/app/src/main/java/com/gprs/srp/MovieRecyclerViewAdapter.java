...
import com.gprs.srp.MovieFragment.OnListFragmentInteractionListener;
...
public class MovieRecyclerViewAdapter
  ...
  public MovieRecyclerViewAdapter(List<MovieItem> items, OnListFragmentInteractionListener listener, List<RecommendationClient.Result> recommendations) {
    values = items;
    this.listener = listener;
  ...
  public void onBindViewHolder(final ViewHolder holder, int position) {
    ...
    if(movies!=null) {
      if (movies.contains(values.get(position).title)) {
        holder.movieTitle.setTextColor(Color.parseColor("#D72222"));
      }
      else
        holder.movieTitle.setTextColor(Color.parseColor("#555555"));
    }
